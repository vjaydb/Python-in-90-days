...
for i, bmi in enumerate(bmi_values,1):
    if bmi <16:
        print(f"{i}:{bmi} : Severely Underweight")
    if bmi >=16 and bmi <= 18.4:
        print(f"{i}:{bmi} : Underweight")
    if bmi >=18.5 and bmi <=24.9:
        print (f"{i}:{bmi} : Normal")
    if bmi >=25 and bmi <=29.9:
        print (f"{i}:{bmi} : Overweight")
    if bmi >=30 and bmi <=34.9:
        print (f"{i}:{bmi} : Obsese Class 1")
    if bmi >=35 and bmi <=39.9:
        print (f"{i}:{bmi} : Obese Class 2")
    if bmi >=40:
        print (f"{i}:{bmi} : Severely Obese")
